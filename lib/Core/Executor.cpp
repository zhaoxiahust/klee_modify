...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
  ...
  if (!isSeeding) {
    if (replayPath && !isInternal) {
		  ...
		  ///////////////////////////////kain
		  printf("into !isSeeding------replayPayh\n");
		  ////////////////////////////////////
  ...
  if (isSeeding && 
      ...
      res == Solver::Unknown) {
		  ///////////////////////////////kain
		  printf("into isSeeding---------current.forkDisabled\n");
		  ////////////////////////////////////
 ...
 if((res ==Solver::True && SymFlag == 2) || SymFlag > 20)///////////分支遍历了20次，为for打算
  {
		 res = Solver::False;///////////////如果已经遍历了分支，而且分支为Ture，则默认flase
		 SymFlag = 0;
  }
  if(res == Solver::True || res == Solver::False)
  {
	    ConstraintFlag = 1;	 //////不添加constraint	  
  }
  else
  {
		ConstraintFlag = 0;		  
  ...
  if (res==Solver::True && SymFlag != 1) { ////////////////////==1则说明不存在，两种情况都要考虑
    if (!isInternal) {                           ////////////////////==2则说明已经遍历了一次，不用在继续遍历了
      if (pathWriter) {
        current.pathOS << "1";
      }
    }
  ...
  } else if (res==Solver::False && SymFlag != 1) {
    if (!isInternal) {
      if (pathWriter) {
        current.pathOS << "0";
      }
    }
	std::cout << "flasekkkkkkkkkkkkkk " << "\n";
	std::cout<<"instruction is " << (current.pc)->inst->getOpcodeName()<<"\n";
...
void Executor::executeCall(ExecutionState &state, 
  ...
  if (f && f->isDeclaration()) {
	...
	//
	/////////////////////////////////////////////////kain////////
   std::cout<<"not into callExternalFunction       "<<f->getNameStr()<<"\n";
   ///////////////////////////////////////////////////////////////
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  Instruction *i = ki->inst;
  /////////////////////////////kain
  std::cout<<"state weight is " << state.weight << "\n";
  printf("execute Instruciton is %s\n", i->getOpcodeName());
  ...
  case Instruction::Br: {
	...
	////////////////////////////////////////////////kain判断
	  std::map<llvm::Instruction *, int>::iterator iter = BranchFlag.find(i);
	  if (iter == BranchFlag.end())
	  {
			BranchFlag.insert(std::pair<llvm::Instruction *, int>(i, 0));		  
			SymFlag = 1;
	  }
	  else
	  {
			SymFlag = 2; ///////////已经执行过了分支
			int n = iter->second;
			BranchFlag.erase(iter);
			BranchFlag.insert(std::pair<llvm::Instruction *, int>(i, n+1));		  
			if(n > 20)
				SymFlag = 21;
	  }
...
//////////////////////////////////测试pthread之类的函数是否走的这条分支
//	std::cout<<"Function into the direct call" << f->getNameStr()<<"\n";
///////////////////////////////
...
//////////////////////////////////测试哪类的函数是否走的这条分支
//	std::cout<<"Function into the un-direct call" << f->getNameStr()<<"\n";
///////////////////////////////
...
void Executor::callExternalFunction(ExecutionState &state,
  ...
  /////////////////////////////////kain//////////////////
   std::cout<<"int externalDispatcher call"<<function->getNameStr()<<"\n";
  ////////////////////////////////////////////////p
...
///////////////////////////////////////kain注释了，误认为执行了外部函数
 ...
 // bool success = 1;
  ...
  ////////////////////////////////kain///////////////////////////////////
   std::cout<<"out externalDispatcher call"<<function->getNameStr()<<"\n";
  /////////////////////////////////////////////////////////////////////
  if (!success) {
...
Executor::StatePair 
Executor::pthread_fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
  Solver::Validity res;
  std::map< ExecutionState*, std::vector<SeedInfo> >::iterator it = 
    seedMap.find(&current);
  bool isSeeding = it != seedMap.end();
