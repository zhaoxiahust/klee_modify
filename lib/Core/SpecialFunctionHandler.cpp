...
#include "CoreStats.h" ///////add by kain
#include "PTree.h"
...
#define addDNR(name, handler) { name, \
  ...
  add("pthread_create", handlePthread_create, false),
...
void SpecialFunctionHandler::handlePthread_create(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments) {
		Function *f;
		...
		printf("into handler thread\n");
	/////////////////////解析第三个函数指针参数
		ref<ConstantExpr> value = cast<ConstantExpr>(arguments[2]);
		f = (Function *)(value->getZExtValue());
		KFunction *kf = executor.kmodule->functionMap[f];

		ExecutionState *threadState, *mainState = &state;

///////////////////////////////////////////实验函数调用方式
/*
		state.pushFrame(state.prevPC, kf);
		state.pc = kf->instructions;
		executor.bindArgument(kf,0,state,arguments[3]);

		if(executor.statsTracker)
				executor.statsTracker->framePushed(state, &state.stack[state.stack.size()-2]);
*/
/////////////////////////////////////////////
...
/////////////////////////////创建新的状态
		...
		++stats::forks;
		threadState = mainState->branch();
		threadState->prevPC=threadState->pc = kf->instructions;
		/////////////////////////////////////构建stack-------解决memory问题,反复执行main
//		threadState->popFrame();
		threadState->stack.clear();
		threadState->pushFrame(0, kf);
//		std::cout<<"out pushFrame()" << "\n";
//		threadState->pushFrame(threadState->prevPC, kf);
//		if(executor.statsTracker)
//				executor.statsTracker->framePushed(*threadState, &(threadState->stack[threadState->stack.size()-1]));
//		if(executor.statsTracker)
//				executor.statsTracker->framePushed(*threadState, 0);
		
//		if(executor.statsTracker && state.stack.back().kf->trackCoverage)
//				executor.statsTracker->markBranchVisited(mainState, threadState);
//		std::cout<<"out statsFrame()" << "\n";
				//////////////////////////////////////

		state.ptreeNode->data=0;
		std::pair<PTree::Node*,PTree::Node*> res =
			executor.processTree->split(state.ptreeNode, threadState, mainState);
		threadState->ptreeNode = res.first;
		mainState->ptreeNode = res.second;

		
///////////////////参数绑定，状态添加
	//	executor.transferToBasicBlock((*(kf->instructions))->getParent(), )
		executor.bindArgument(kf,0,*threadState,arguments[3]);
		executor.addedStates.insert(threadState);

		std::cout<<"out thread handler" << "\n";
}
void SpecialFunctionHandler::handlePuts(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments) {
